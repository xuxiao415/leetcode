...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[] shortPathNew = dijkstraNew(weight1, start);

        for (int i = 0; i < shortPathNew.length; i++) {
            System.out.println("从" + start + "出发到" + i + "的最短距离为：" + shortPathNew[i]);
        }
    ...
    public static int[] dijkstraNew(int[][] weight, int start) {
        // 接受一个有向图的权重矩阵，和一个起点编号start（从0编号，顶点存在数组中）
        // 返回一个int[] 数组，表示从start到它的最短路径长度
        int n = weight.length; // 顶点个数
        int[] shortPath = new int[n]; // 保存start到其他各点的最短路径
